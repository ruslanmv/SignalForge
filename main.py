...
def load_config():
    ...
    print(f"Configuration fileloaded successfully: {config_path}")
    ...
    # Notification channel config (env vars priority)
    ...
    if notification_sources:
        print(f"Notification channel config source: {', '.join(notification_sources)}")
    else:
...
CONFIG = load_config()
print(f"SignalForge v{VERSION} Configuration loaded")
        ...
        for record_file in self.record_dir.glob("push_record_*.json"):
            try:
                ...
                if (current_time - file_date).days > retention_days:
                    record_file.unlink()
                    print(f"Cleaning expired push records: {record_file.name}")
            except Exception as e:
        ...
        while retries <= max_retries:
            try:
                ...
                if retries <= max_retries:
                    ...
                    print(f"Request {id_value} failed: {e}. {wait_time:.2f}sec retry...")
        ...
        for i, id_info in enumerate(ids_list):
            ...
            if response:
                try:
                    ...
                    print(f"Parsing {id_value} response failed")
...
def parse_file_titles(file_path: Path) -> Tuple[Dict, Dict]:
    """Parse title data from single txt file, returns (titles_by_id, id_to_name)"""
    titles_by_id = {}
    id_to_name = {}
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        ...
        for section in sections:
            if not section.strip() or "==== The following IDs failed to fetch ====" in section:
                continue
...
def read_all_today_titles(
    ...
    """Read all title files for today, supports filtering by current monitoring platforms"""
    date_folder = format_date_folder()
    txt_dir = Path("output") / date_folder / "txt"
...
def detect_latest_new_titles(current_platform_ids: Optional[List[str]] = None) -> Dict:
    """Detect latest batch of new titles for today, supports filtering by current monitoring platforms"""
    date_folder = format_date_folder()
    txt_dir = Path("output") / date_folder / "txt"
    ...
    # If current platform list specified, filter latest file data
    if current_platform_ids is not None:
...
def calculate_news_weight(
    ...
    # Rank weight: Σ(11 - min(rank, 10)) / appearance count
    rank_scores = []
    for rank in ranks:
    ...
    # Hotness bonus: high rank count / total appearance count × 100
...
def matches_word_groups(
    ...
    # Defensive type check: ensure title is valid string
    if not isinstance(title, str):
...
def count_word_frequency(
    ...
    """Count word frequency, supports required words, frequency words, filter words, and mark new titles"""

    # If no word groups configured, create a virtual group containing all news
    if not word_groups:
        print("Frequency words config empty, will display all news")
    ...
    # Determine data source to process and new marking logic
    if mode == "incremental":
        if is_first_today:
            # Incremental mode + first crawl of day: process all news, all marked as new
            results_to_process = results
            all_news_are_new = True
        else:
            # Incremental mode + not first crawl of day: only process new news
            results_to_process = new_titles if new_titles else {}
            all_news_are_new = True
    elif mode == "current":
        # Current mode: only process current time batch news, but stats come from all history
        if title_info:
            latest_time = None
    ...
    for source_id, titles_data in results_to_process.items():
        ...
        for title, title_data in titles_data.items():
            ...
            # If incremental mode or current mode first crawl, count matched new news
            ...
            for group in word_groups:
                ...
                # If "All News" mode, all titles match the first (only) group
                if len(word_groups) == 1 and word_groups[0]["group_key"] == "All News":
                    group_key = group["group_key"]
                ...
                # For current mode, fetch complete data from historical stats
                if (
                    mode == "current"
                    and title_info
                ...
                is_new = False
                if all_news_are_new:
                    # In incremental mode all processed news are new, or first crawl of day all news are new
                    is_new = True
    ...
    if mode == "incremental":
        if is_first_today:
            ...
            print(
                f"Incremental mode: first crawl of day, {total_input_news}  news items have {matched_new_count}  items {filter_status}"
            )
        else:
            if new_titles:
                ...
                filter_status = (
                    ...
                    else "matching frequency words"
                )
                print(
                    f"Incremental mode: {total_new_count}  new news items,  {matched_new_count}  items {filter_status}"
                )
        ...
        if is_first_today:
            ...
            print(
                f"Current ranking mode: first crawl of day, {total_input_news}  current ranking news items have {matched_new_count}  items {filter_status}"
            )
        else:
            ...
            print(
                f"Current ranking mode: {total_input_news}  current ranking news items have {matched_count}  items {filter_status}"
            )
    ...
    # Choose sort priority based on config
    if CONFIG.get("SORT_BY_POSITION_FIRST", False):
        # Sort by config position first, then by hot item count
        stats.sort(key=lambda x: (x["position"], -x["count"]))
    else:
        # Sort by hot item count first, then by config position (original logic)
...
def render_html_content(
    ...
    if report_data["failed_ids"]:
        html += """
                <div class="error-section">
                    <div class="error-title">⚠️ Request Failed Platforms</div>
                    <ul class="error-list">"""
    ...
    if report_data["stats"]:
        ...
        for i, stat in enumerate(report_data["stats"], 1):
            ...
            # Process news titles under each word group, numbering each news item
    ...
    if report_data["new_titles"]:
        html += f"""
                <div class="new-section">
                    <div class="new-section-title">New Hot Topics This Round (Total {report_data['total_new_count']}  items)</div>"""
        ...
        for source_data in report_data["new_titles"]:
            ...
            # Add numbering for new news as well
            for idx, title_data in enumerate(source_data["titles"], 1):
                ranks = title_data.get("ranks", [])

                # Process new news rank display
                rank_class = ""
                if ranks:
                    min_rank = min(ranks)
                    if min_rank <= 3:
    ...
    html += """
            ...
            <div class="footer">
                <div class="footer-content">
                    Powered by <span class="project-name">SignalForge</span> generated · 
                    <a href="https://github.com/ruslanmv/SignalForge" target="_blank" class="footer-link">
        ...
        <script>
            async function saveAsImage() {
                ...
                try {
                    ...
                    // Wait again to ensure button is completely hidden
                    ...
                    const now = new Date();
                    const filename = `SignalForge_Hot News Analysis_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}.png`;
            ...
            async function saveAsMultipleImages() {
                ...
                try {
                    ...
                    // Fetch all possible split elements
                    ...
                    // Group process news-items by word-group
                    ...
                    // Download all images
                    const now = new Date();
                    const baseFilename = `SignalForge_Hot News Analysis_${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}_${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}`;
                    ...
                    console.error('Save in segments failed:', error);
                    const buttons = document.querySelector('.save-buttons');
                    buttons.style.visibility = 'visible';
                    button.textContent = 'Save failed';
                    setTimeout(() => {
...
def render_feishu_content(
    ...
    if not text_content:
        if mode == "incremental":
            mode_text = "Incremental mode: no new matching hot keywords"
        elif mode == "current":
            mode_text = "Current ranking mode: no matching hot keywords"
        else:
            mode_text = "No matching hot keywords"
        text_content = f"📭 {mode_text}\n\n"

    if report_data["new_titles"]:
        if text_content and "no matching hot keywords" not in text_content:
            text_content += f"\n{CONFIG['FEISHU_MESSAGE_SEPARATOR']}\n\n"

        text_content += (
            f"🆕 **New Hot News This Time** (Total {report_data['total_new_count']}  items)\n\n"
        )
    ...
    if report_data["failed_ids"]:
        ...
        text_content += "⚠️ **Data Fetch Failed Platforms:**\n\n"
    ...
    if update_info:
        text_content += f"\n<font color='grey'>SignalForge new version found {update_info['remote_version']}, current {update_info['current_version']}</font>"
...
def render_dingtalk_content(
    ...
    text_content += f"**Type: ** Hot Topic Analysis Report\n\n"
    ...
    if update_info:
        text_content += f"\n> SignalForge new version found **{update_info['remote_version']}**, current **{update_info['current_version']}**"
...
def split_content_into_batches(
    ...
    """Batch process message content ensuring word group title + at least first news integrity"""
    ...
    if format_type == "wework":
        ...
        base_header += f"**Type: ** Hot Topic Analysis Report\n\n"
        base_header += "---\n\n"
    ...
    if report_data["stats"]:
        ...
        # Process each word group (ensure word group title + first news item atomicity)
        for i, stat in enumerate(report_data["stats"]):
            word = stat["word"]
            count = stat["count"]
            sequence_display = f"[{i + 1}/{total_count}]"
            ...
            # Atomicity check: word group title + first news item must be processed together
    ...
    # Process new news (also ensure source title + first news item atomicity)
    if report_data["new_titles"]:
        new_header = ""
        if format_type == "wework":
            new_header = f"\n\n\n\n🆕 **New Hot News This Time** (Total {report_data['total_new_count']}  items)\n\n"
        elif format_type == "telegram":
            new_header = (
                f"\n\n🆕 New Hot News This Round (Total {report_data['total_new_count']}  items)\n\n"
            )
        elif format_type == "ntfy":
            new_header = f"\n\n🆕 **New Hot News This Time** (Total {report_data['total_new_count']}  items)\n\n"
        elif format_type == "feishu":
        ...
        for source_data in report_data["new_titles"]:
            ...
            # Atomicity check: source title + first news item
...
def send_to_notifications(
    ...
    if CONFIG["PUSH_WINDOW"]["ENABLED"]:
        ...
        if not push_manager.is_in_time_range(time_range_start, time_range_end):
            now = get_beijing_time()
            print(
                f"Push window control: current time {now.strftime('%H:%M')} not in push window {time_range_start}-{time_range_end}, skipping push"
            )
            return results
    ...
    # If successfully sent any notification and once per day enabled, record push
    if (
...
def send_to_feishu(
    ...
    # Fetching分批内容，使用Feishu专用的批 times大小
    ...
    print(f"Feishu消息分为 {len(batches)} batches sent [{report_type}]")

    # 逐批发送
    for i, batch_content in enumerate(batches, 1):
        batch_size = len(batch_content.encode("utf-8"))
        print(
            f"发送Feishu第 {i}/{len(batches)} batch, size:{batch_size} bytes [{report_type}]"
        )

        # 添加批 times标识
        if len(batches) > 1:
            batch_header = f"**[第 {i}/{len(batches)} 批 times]**\n\n"
            # 将批 times标识插入到适当位置（在统计标题之后）
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("StatusCode") == 0 or result.get("code") == 0:
                    print(f"Feishu第 {i}/{len(batches)} batches sentsuccess [{report_type}]")
                    # 批 times间间隔
                    if i < len(batches):
                    ...
                    print(
                        f"Feishu第 {i}/{len(batches)} batches sentfailed [{report_type}]，错误：{error_msg}"
                    )
                    return False
            else:
                print(
                    f"Feishu第 {i}/{len(batches)} batches sentfailed [{report_type}]，Status code:{response.status_code}"
                )
                return False
        except Exception as e:
            print(f"Feishu第 {i}/{len(batches)} batches sent出错 [{report_type}]：{e}")
            return False

    print(f"FeishuAll {len(batches)} batches sent完成 [{report_type}]")
    return True
...
def send_to_dingtalk(
    ...
    # Fetching分批内容，使用DingTalk专用的批 times大小
    batches = split_content_into_batches(
        report_data,
        "dingtalk",
        update_info,
    ...
    print(f"DingTalk消息分为 {len(batches)} batches sent [{report_type}]")

    # 逐批发送
    for i, batch_content in enumerate(batches, 1):
        batch_size = len(batch_content.encode("utf-8"))
        print(
            f"发送DingTalk第 {i}/{len(batches)} batch, size:{batch_size} bytes [{report_type}]"
        )

        # 添加批 times标识
        if len(batches) > 1:
            batch_header = f"**[第 {i}/{len(batches)} 批 times]**\n\n"
            # 将批 times标识插入到适当位置（在标题之后）
            if "📊 **Hot Keywords Statistics**" in batch_content:
                batch_content = batch_content.replace(
                    "📊 **Hot Keywords Statistics**\n\n", f"📊 **Hot Keywords Statistics** {batch_header}\n\n"
                )
        ...
        payload = {
            "msgtype": "markdown",
            "markdown": {
                "title": f"SignalForge Hot Topic Analysis Report - {report_type}",
                "text": batch_content,
            },
        }
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("errcode") == 0:
                    print(f"DingTalk第 {i}/{len(batches)} batches sentsuccess [{report_type}]")
                    # 批 times间间隔
                    if i < len(batches):
                        time.sleep(CONFIG["BATCH_SEND_INTERVAL"])
                else:
                    print(
                        f"DingTalk第 {i}/{len(batches)} batches sentfailed [{report_type}]，错误：{result.get('errmsg')}"
                    )
                    return False
            else:
                print(
                    f"DingTalk第 {i}/{len(batches)} batches sentfailed [{report_type}]，Status code:{response.status_code}"
                )
                return False
        except Exception as e:
            print(f"DingTalk第 {i}/{len(batches)} batches sent出错 [{report_type}]：{e}")
            return False

    print(f"DingTalkAll {len(batches)} batches sent完成 [{report_type}]")
    return True
...
def send_to_wework(
    ...
    """发送到WeCom（支持分批发送，支持 markdown 和 text 两种格式）"""
    headers = {"Content-Type": "application/json"}
    proxies = None
    if proxy_url:
        proxies = {"http": proxy_url, "https": proxy_url}

    # Fetching消息类型配置（markdown 或 text）
    ...
    print(f"WeCom消息分为 {len(batches)} batches sent [{report_type}]")
    ...
    for i, batch_content in enumerate(batches, 1):
        ...
        print(
            f"发送WeCom第 {i}/{len(batches)} batch, size:{batch_size} bytes [{report_type}]"
        )
        ...
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                if result.get("errcode") == 0:
                    print(f"WeCom第 {i}/{len(batches)} batches sentsuccess [{report_type}]")
                    # 批 times间间隔
                    if i < len(batches):
                        time.sleep(CONFIG["BATCH_SEND_INTERVAL"])
                else:
                    print(
                        f"WeCom第 {i}/{len(batches)} batches sentfailed [{report_type}]，错误：{result.get('errmsg')}"
                    )
                    return False
            else:
                print(
                    f"WeCom第 {i}/{len(batches)} batches sentfailed [{report_type}]，Status code:{response.status_code}"
                )
                return False
        except Exception as e:
            print(f"WeCom第 {i}/{len(batches)} batches sent出错 [{report_type}]：{e}")
            return False

    print(f"WeComAll {len(batches)} batches sent完成 [{report_type}]")
    return True
...
def send_to_telegram(
    ...
    print(f"Telegram message divided into {len(batches)} batches sent [{report_type}]")

    # 逐批发送
    for i, batch_content in enumerate(batches, 1):
        batch_size = len(batch_content.encode("utf-8"))
        print(
            f"Sending Telegram batch {i}/{len(batches)} batch, size:{batch_size} bytes [{report_type}]"
        )

        # 添加批 times标识
        if len(batches) > 1:
            batch_header = f"<b>[第 {i}/{len(batches)} 批 times]</b>\n\n"
            batch_content = batch_header + batch_content
        ...
        try:
            ...
            if response.status_code == 200:
                result = response.json()
                if result.get("ok"):
                    print(f"Telegram batch {i}/{len(batches)} batches sentsuccess [{report_type}]")
                    # 批 times间间隔
                    if i < len(batches):
                        time.sleep(CONFIG["BATCH_SEND_INTERVAL"])
                else:
                    print(
                        f"Telegram batch {i}/{len(batches)} batches sentfailed [{report_type}]，错误：{result.get('description')}"
                    )
                    return False
            else:
                print(
                    f"Telegram batch {i}/{len(batches)} batches sentfailed [{report_type}]，Status code:{response.status_code}"
                )
                return False
        except Exception as e:
            print(f"Telegram batch {i}/{len(batches)} batches sent出错 [{report_type}]：{e}")
            return False

    print(f"TelegramAll {len(batches)} batches sent完成 [{report_type}]")
    return True
...
def send_to_email(
    ...
    try:
        ...
        # 设置Email主题
        now = get_beijing_time()
        subject = f"SignalForge Hot Topic Analysis Report - {report_type} - {now.strftime('%m-%d %H:%M')}"
        ...
        # 添加纯文本部分（作为备选）
        text_content = f"""
SignalForge Hot Topic Analysis Report
========================
报告Type: {report_type}
generatedTime: {now.strftime('%Y-%m-%d %H:%M:%S')}

请使用支持HTML的Email客户端查看完整报告内容。
        """
        ...
        try:
            ...
            print(f"Email发送success [{report_type}] -> {to_email}")
            return True

        except smtplib.SMTPServerDisconnected:
            print(f"Email发送failed：服务器意外断开连接，请检查网络或稍后重试")
            return False

    except smtplib.SMTPAuthenticationError as e:
        print(f"Email发送failed：认证错误，请检查邮箱和密码/授权码")
        print(f"详细错误: {str(e)}")
        return False
    except smtplib.SMTPRecipientsRefused as e:
        print(f"Email发送failed：收件人地址被拒绝 {e}")
        return False
    except smtplib.SMTPSenderRefused as e:
        print(f"Email发送failed：发件人地址被拒绝 {e}")
        return False
    except smtplib.SMTPDataError as e:
        print(f"Email发送failed：Email数据错误 {e}")
        return False
    except smtplib.SMTPConnectError as e:
        print(f"Email发送failed：无法连接到 SMTP 服务器 {smtp_server}:{smtp_port}")
        print(f"详细错误: {str(e)}")
        return False
    except Exception as e:
        print(f"Email发送failed [{report_type}]：{e}")
        import traceback
...
def send_to_ntfy(
    ...
    report_type_en_map = {
        ...
        "current榜单汇总": "Current Ranking",
        "增量更新": "Incremental Update",
        "Real-time incremental": "Realtime Incremental", 
        "实时current榜单": "Realtime Current Ranking",  
    }
    ...
    # Fetching分批内容，使用ntfy专用的4KB限制
    batches = split_content_into_batches(
        report_data, "ntfy", update_info, max_bytes=3800, mode=mode
    )

    total_batches = len(batches)
    print(f"ntfy message divided into {total_batches} batches sent [{report_type}]")

    # 反转批 times顺序，使得在ntfy客户端显示时顺序正确
    # ntfy显示最新消息在上面，所以我们从最后一批开始推送
    reversed_batches = list(reversed(batches))
    
    print(f"ntfy will push in reverse order (last batch first) for correct client display order")

    # 逐批发送（反向顺序）
    success_count = 0
    for idx, batch_content in enumerate(reversed_batches, 1):
        # 计算正确的批 times编号（用户视角的编号）
        actual_batch_num = total_batches - idx + 1
        
        batch_size = len(batch_content.encode("utf-8"))
        print(
            f"Sending ntfy batch {actual_batch_num}/{total_batches} batch (push order: {idx}/{total_batches}），大小：{batch_size} bytes [{report_type}]"
        )

        # 检查消息大小，确保不超过4KB
        if batch_size > 4096:
            print(f"警告：ntfy batch {actual_batch_num} batch message too large ({batch_size} bytes），可能被拒绝")

        # 添加批 times标识（使用正确的批 times编号）
        current_headers = headers.copy()
        if total_batches > 1:
            batch_header = f"**[第 {actual_batch_num}/{total_batches} 批 times]**\n\n"
            batch_content = batch_header + batch_content
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"ntfy batch {actual_batch_num}/{total_batches} batches sentsuccess [{report_type}]")
                success_count += 1
                if idx < total_batches:
                    # 公Total服务器建议 2-3 秒，自托管可以更短
                    interval = 2 if "ntfy.sh" in server_url else 1
                    time.sleep(interval)
            elif response.status_code == 429:
                print(
                    f"ntfy batch {actual_batch_num}/{total_batches} batch rate limited [{report_type}], waiting before retry"
                )
                time.sleep(10)  # 等待10sec retry
                # 重试一 times
                ...
                if retry_response.status_code == 200:
                    print(f"ntfy batch {actual_batch_num}/{total_batches} 批 times重试success [{report_type}]")
                    success_count += 1
                else:
                    print(
                        f"ntfy batch {actual_batch_num}/{total_batches} 批 times重试failed，Status code:{retry_response.status_code}"
                    )
            elif response.status_code == 413:
                print(
                    f"ntfy batch {actual_batch_num}/{total_batches} batch message rejected for being too large [{report_type}], message size: {batch_size} bytes"
                )
            else:
                print(
                    f"ntfy batch {actual_batch_num}/{total_batches} batches sentfailed [{report_type}]，Status code:{response.status_code}"
                )
                try:
            ...
            print(f"ntfy batch {actual_batch_num}/{total_batches} 批 times连接超时 [{report_type}]")
        except requests.exceptions.ReadTimeout:
            print(f"ntfy batch {actual_batch_num}/{total_batches} 批 times读取超时 [{report_type}]")
        except requests.exceptions.ConnectionError as e:
            print(f"ntfy batch {actual_batch_num}/{total_batches} 批 times连接错误 [{report_type}]：{e}")
        except Exception as e:
            print(f"ntfy batch {actual_batch_num}/{total_batches} batches sent异常 [{report_type}]：{e}")

    # 判断整体发送是否success
    if success_count == total_batches:
        print(f"ntfyAll {total_batches} batches sent完成 [{report_type}]")
        return True
    elif success_count > 0:
        print(f"ntfy部分发送success：{success_count}/{total_batches} 批 times [{report_type}]")
        return True  # 部分success也视为success
    else:
        print(f"ntfy发送完全failed [{report_type}]")
        return False
...
class NewsAnalyzer:
    ...
    MODE_STRATEGIES = {
        "incremental": {
            ...
            "description": "Incremental mode (only new news, no push when no new items)",
        ...
        "current": {
            "mode_name": "current榜单模式",
            "description": "current榜单模式（current榜单匹配新闻 + 新增新闻区域 + 按时推送）",
            "realtime_report_type": "实时current榜单",
            "summary_report_type": "current榜单汇总",
            "should_send_realtime": True,
            "should_generate_summary": True,
        ...
        "daily": {
            ...
            "description": "Daily summary mode（All匹配新闻 + 新增新闻区域 + 按时推送）",
    ...
    def _check_version_update(self) -> None:
        ...
        try:
            ...
            if need_update and remote_version:
                ...
                print("Version check complete, current version is latest")
        except Exception as e:
    ...
    def _has_valid_content(
        ...
        if self.report_mode in ["incremental", "current"]:
            ...
            # Daily summary mode下，检查是否有匹配的频率词新闻或新增新闻
    ...
    def _load_analysis_data(
        ...
        """Unified data loading and preprocessing, filter historical data using current monitoring platform list"""
        try:
            # FetchingcurrentConfigured monitoring platformsID列表
    ...
    def _send_notification_if_needed(
        ...
        """Unified notification sending logic with all judgment conditions"""
        ...
        if (
            ...
            print("⚠️ 警告：通知功能已启用但No notification channels configured，将跳过通知发送")
    ...
    def _generate_summary_report(self, mode_strategy: Dict) -> Optional[str]:
        ...
        summary_type = (
            "current榜单汇总" if mode_strategy["summary_mode"] == "current" else "Daily summary"
        )
    ...
    def _generate_summary_html(self, mode: str = "daily") -> Optional[str]:
        ...
        summary_type = "current榜单汇总" if mode == "current" else "Daily summary"
    ...
    def _initialize_and_check_config(self) -> None:
        """Common initialization and configuration check"""
        now = get_beijing_time()
        print(f"Current Beijing time: {now.strftime('%Y-%m-%d %H:%M:%S')}")

        if not CONFIG["ENABLE_CRAWLER"]:
            print("Crawler disabled (ENABLE_CRAWLER=False), program exits")
            return
        ...
        if not CONFIG["ENABLE_NOTIFICATION"]:
            ...
            print("No notification channels configured，将只进行数据抓取，不发送通知")
        else:
    ...
    def _crawl_data(self) -> Tuple[Dict, Dict, List]:
        ...
        print(f"开始爬取数据，Request间隔 {self.request_interval} milliseconds")
    ...
    def _execute_mode_strategy(
        ...
        # FetchingCurrent monitoring platformsID列表
        ...
        # In current mode, real-time push needs complete historical data for statistics integrity
        if self.report_mode == "current":
            # 加载完整的历史数据（已按current平台过滤）
            analysis_data = self._load_analysis_data()
            if analysis_data:
                (
                    all_results,
        ...
        if mode_strategy["should_generate_summary"]:
            if mode_strategy["should_send_realtime"]:
                # 如果已经发送了实时通知，汇总只generatedHTML不发送通知
                summary_html = self._generate_summary_html(
                    mode_strategy["summary_mode"]
                )
            else:
                # daily模式：直接generated汇总报告并发送通知
        ...
        if self._should_open_browser() and html_file:
            ...
            if summary_html:
                print(f"Summary report generated（Docker环境）: {summary_html}")
            else:
                print(f"HTML report generated（Docker环境）: {html_file}")
